<?php
  ...
  if($_SERVER['REQUEST_METHOD'] == 'POST') {
    //Validaciones
    if(!$_POST['Saldo']) $errores[] = 'Necesita indicar el monto a retirar en cuenta';
    if($_POST['Saldo'] > $cuenta->Saldo) $errores[] = 'No cuenta con el suficiente saldo para retirarlo';

    if(empty($errores)) {
      $NewSaldo = $cuenta->Saldo - $_POST['Saldo'];

      //generamos el registro de transaccion y lo almacenamos en el historial
      $arregloInfoTransaccion['CodigoTipoTransaccion'] = 'RET';
      ...
      $arregloInfoTransaccion['Descripcion'] = 'Retiro de cuenta bancaria';
      ...
      if($Resultado) {
        header('location: /ProyectoCatedra/app/retirar.php?state=1&&NumeroCuenta=' . $cuenta->NumeroCuenta . '&&SaldoDescontado=' . $_POST['Saldo'] . '');
      }
      else {
...
<main>
  <div class="Contenedor-TituloAdmin gr">
    <h2 class="TituloAdmin gr">Retirar de cuenta</h2>
  </div>
  ...
  <div class="Contenedor-Errores">
        ...
        <?php endforeach ?>
        <?php if(isset($state) AND empty($errores)){ if($state == 1) echo '<div class="Alerta green">Retiro realizado exitosamente</div>'; }?>
    </div>
  ...
  <div class="Contenedor-DEP">
    ...
    <form method="POST" class="NuevoSaldo-DEP">
      ...
      <input type="submit" class="Operacion-Saldo" value="Retirar Saldo">
    </form>
